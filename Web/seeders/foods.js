...
module.exports = ()=>{
    ...
    for(let i = 0 ; i < a.results.length ; i++){
        var food = {
          cooking_method_id: Math.floor((Math.random() * 15) + 1),
          food_group_id: Math.floor((Math.random() * 12) + 1),
          name: a.results[i].title,
          price: Math.floor((Math.random() * 1000) + 100) * 1000,
          image_url: a.results[i].image,
          status: true
        }
        models.Foods.create(food).then(re =>{})
    }
}